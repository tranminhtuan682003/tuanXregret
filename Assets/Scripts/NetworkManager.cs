...
public class NetworkManager : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    public void OnPlayerJoined(NetworkRunner runner, PlayerRef player)
    {
        if (runner.IsServer)
        {
            var networkObject = runner.Spawn(playerPrefab, new Vector3(0, 2.5f, 0), Quaternion.identity, player);
            Debug.Log($"Player {player} joined and spawned at position {networkObject.transform.position}");
        }
    }
    ...
    public void OnInput(NetworkRunner runner, NetworkInput input)
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Moved)
            {
                ...
                {
                    currentDirection = swipeDelta.x > 0 ? Vector3.right : Vector3.left;
                }
                else
                {
                    currentDirection = swipeDelta.y > 0 ? Vector3.forward : Vector3.back;
                }
        ...
        data.direction = currentDirection;
        Debug.Log($"Input direction: {currentDirection}");
        input.Set(data);
    }
